...
def _app_to_dict(app: AppConfig) -> dict[str, Any]:
    ...
    d.update({
        ...
        "deploy": {
            ...
            "cpu_limit": app.deploy.cpu_limit,
            "memory_limit_mb": app.deploy.memory_limit_mb,
        },
        "health": {
            "protocol": app.health.protocol.value,
...
def _parse_app(name: str, data: dict[str, Any]) -> AppConfig:
    ...
    return AppConfig(
        ...
        deploy=DeployConfig(
            ...
            cpu_limit=float(dep.get("cpu_limit", 0.0)),
            memory_limit_mb=int(dep.get("memory_limit_mb", 0)),
        ),
        health=HealthConfig(
            protocol=HealthProtocol(hlth.get("protocol", "http")),
