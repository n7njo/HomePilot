"""Add App wizard — step-by-step registration of a new app."""
...
class AddAppScreen(Screen):
    """Wizard to register a new application."""
    ...
    def compose(self) -> ComposeResult:
        ...
        yield VerticalScroll(
            ...
            # Step 1: Name
            Label("  App Name (lowercase, dashes allowed):"),
    ...
    def _auto_detect(self) -> None:
        ...
        # Auto-fill image/container names
    ...
    def _save(self) -> None:
        """Create the new app config and save."""
        status = self.query_one("#wizard-status", Static)

        name = self.query_one("#app-name", Input).value.strip()
        if not name:
            status.update("[red]App name is required[/red]")
            return

        if name in self._config.apps:
            status.update(f"[red]App '{name}' already exists[/red]")
            return
        ...
        try:
            ...
            status.update(f"[green]✅ App '{name}' added successfully![/green]")
